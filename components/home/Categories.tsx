...
export default function Categories() {
  ...
  return (
    <section className="py-16 lg:py-20 bg-earthy-brown">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {categories.map((category) => (
            <Link
              key={category.key}
              href={category.href}
              className="group relative overflow-hidden rounded-2xl bg-pure-white p-8 shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-1"
            >
              {/* Background Gradient */}
              ...
              <div className="relative z-10 text-center">
                {/* Icon */}
                <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-brand-cyan text-pure-white mb-6 group-hover:scale-110 transition-transform duration-300">
                  {category.icon}
                </div>
                ...
                {/* Description */}
                <p className="text-sm text-deep-teal/70 group-hover:text-deep-teal/80 transition-colors">
                  {t(`${category.key}.description`)}
                </p>

                {/* Hover Arrow */}
                <div className="mt-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <svg className="mx-auto w-6 h-6 text-deep-teal/70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
